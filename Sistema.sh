...
if [ ! -f "$ARCHIVO_CONTRASENAS" ]; then
    ...
    if id "$usuario_admin" &>/dev/null; then {
        ...
        userdel -r "$usuario_admin" 2>/dev/null
...
function crear_informe() {
    ...
    if [ -e "$archivo_sala" ]; then
        ...
        if sudo touch "$archivo_sala" && sudo chmod 777 "$archivo_sala"; then
...
function mostrar_informacion_sala() {
    ...
    echo "| 4. Listado de las Computadoras                |"
    echo "| 5. Salir                                      |"
    echo "|_______________________________________________|"    
    echo
    ...
    case $opcion_mostrar in
        ...
        clear
        echo " ................................................"
        echo "|  Mostrar Información por Cantidad de Salas  |"
        echo " ................................................"
        echo
        read -p "Ingrese la Cantidad de Salas: " cantidad_salas_mostrar
        mostrar_por_cantidad_salas "$cantidad_salas_mostrar"
        ;;
        4)
...
# Función para mostrar información de salas por cantidad de salas
function mostrar_por_cantidad_salas() {
    local cantidad_salas="$1"
    clear
    echo
    echo "Salas de Informática con Cantidad de Salas: $cantidad_salas"
    echo
    # Buscar y mostrar información de las salas con la cantidad especificada
    for archivo_sala in "$DIRECTORIO_SALAS"/*.txt; do
        if grep -q "Cantidad de Salas: $cantidad_salas" "$archivo_sala"; then
            cat "$archivo_sala"
            echo
        fi
    done
...
# Función para mostrar un listado completo de todas las computadoras
function mostrar_listado_completo() {
    clear
    echo
    echo "Listado Completo de Todas las Computadoras"
    echo
    for archivo_sala in "$DIRECTORIO_SALAS"/*.txt; do
        cat "$archivo_sala"
        echo
