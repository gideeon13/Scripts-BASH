...
imprimir_titulo() {
    echo -e "${AZUL} __________________________________________________"
    echo -e "|${ROJO}                                                  ${AZUL}|"
    echo -e "|${ROJO}                   Bienvenido al                  ${AZUL}|"
    echo -e "|${ROJO} Sistema de Relevamiento de Salas de Informática  ${AZUL}|"
    echo -e "|${ROJO}__________________________________________________${AZUL}|"
    echo -e "${RESET}"
}
...
# Función para verificar si el archivo de contraseñas está creado
function verificar_archivo_contrasenas() {
    if [ ! -f "$ARCHIVO_CONTRASENAS" ]; then
        echo "El archivo de contraseñas no está creado."
        echo "Por favor, cree la estructura del sistema antes de continuar."
        pausa
        return 1  # Devuelve 1 para indicar que el archivo no está creado
    fi
    return 0  # Devuelve 0 para indicar que el archivo está creado
}
...
function ingresar_sistema() {
    ...
    imprimir_titulo
    echo
    echo "Usuario del sistema operativo: ${AMARILLO}$usuario_actual${RESET}"
    read -p "Ingrese su contraseña del sistema: " contrasena_sistema

    # Verificar las credenciales del usuario
    if [ "$(id -nu)" == "$usuario_actual" ] && [ "$(echo "$usuario_actual:$contrasena_sistema" | chpasswd -e)" == "Success" ]; then
        clear
        echo "Iniciaste sesión exitosamente como $usuario_actual."
        pausa
...
function menu_root() {
  while true; do
    ...
    case $opcion_root in
      1) 
        # Verificar si el archivo de contraseñas está creado
